...
function drawScene(
  ...
  ctx.setLineDash([6, 6]);
  ties.forEach((t) => {
    const a = points.find((p) => p.id === t.p1),
      b = points.find((p) => p.id === t.p2);
    if (!a || !b) return;
    const s1 = W2S(a),
      s2 = W2S(b);
    ctx.beginPath();
...
export default function Drawing2D() {
  ...
  const sliderToZoom = (s) =>
    Math.min(MAX_Z, Math.max(MIN_Z, BASE_ZOOM * Math.pow(2, s / 10)));
  const zoomToSlider = (z) =>
    Math.max(
      MIN_S,
      Math.min(MAX_S, Math.round(10 * Math.log2((z || BASE_ZOOM) / BASE_ZOOM) * 100) / 100)
    );
  useEffect(() => {
    setSval(zoomToSlider(zoom));
  }, [zoom]);
  ...
  useEffect(() => () => tempTimerRef.current && clearTimeout(tempTimerRef.current), []);
  ...
  const addPoint = () => {
    ...
    const id = safeId();
    const pt = { id, label: nextLabel(), x, y, h: hVal };
  ...
  const onPointerMove = (e) => {
    ...
    if (pts.length === 1) {
      ...
      setZoom((z) => {
        ...
        const sx = w / 2 + wx * nz + tx;
        const sy = h / 2 - wy * nz + ty;
        setTx((v) => v + (mid.x - sx));
        setTy((v) => v + (mid.y - sy));
        return nz;
      });
  ...
  const onPointerUp = (e) => {
    ...
    const world = {
      x: (mx - sizeRef.current.wCss / 2 - tx) / zoom,
    ...
    if (mode === "eraseLine") {
      ...
      lines.forEach((ln, idx) => {
        ...
        const t = Math.max(
          ...
          Math.min(
            1,
            ((world.x - a.x) * vx + (world.y - a.y) * vy) / (vx * vx + vy * vy || 1)
          )
        );
        const cx = a.x + t * vx,
          cy = a.y + t * vy;
    ...
    setSelected((sel) => {
      ...
      if (mode === "line" && ids.length === 2) {
        const [aId, bId] = ids;
        if (aId !== bId) setLines((ls) => [...ls, { id: safeId(), p1: aId, p2: bId }]);
        return [];
      }
      ...
      if (mode === "refLine") {
        // two picks set/overwrite ref; third point ⇒ E/N measure from FIRST pick
        if (!refLine && ids.length === 2) {
          setRefLine({ aId: ids[0], bId: ids[1] });
          return [];
        }
        if (refLine && ids.length === 2) {
          setRefLine({ aId: ids[0], bId: ids[1] }); // overwrite by picking 2 again
          return [];
        }
        if (refLine) {
          ...
          const t =
            ((c.x - a.x) * vx + (c.y - a.y) * vy) / (abLen * abLen);
          const px = a.x + t * vx,
            py = a.y + t * vy;
          const perp = Math.hypot(c.x - px, c.y - py);
          const crossZ = vx * (c.y - a.y) - vy * (c.x - a.x);
          const ESigned = (crossZ >= 0 ? -1 : 1) * perp; // left=+ conv flipped per request
      ...
      if (mode === "tie" && ids.length === 2) {
        ...
        if (aId !== bId) {
          ...
          const e = +(b.x - a.x).toFixed(2);
          const n = +(b.y - a.y).toFixed(2);
          const h = +((b.h || 0) - (a.h || 0)).toFixed(2);
      ...
      if (mode === "circle") {
        if (ids.length === 1) return ids; // center only (no popup texts)
        if (ids.length === 2) {
  ...
  const linesView = lines.map((l) => {
    ...
    return {
      ...l,
      name: lineName(a?.label, b?.label),
      len: a && b ? Math.round(dist2D(a, b)) : 0,
    };
  });
  ...
  const hGroups = (() => {
    ...
    return Array.from(mp.entries()).map(([h, arr]) => ({
      h: +h,
      count: arr.length,
        ...
        <div className="row" style={{ gap: 6, flexWrap: "wrap" }}>
          ...
          {refLine && (
            <>
              ...
              >
                Ref: {points.find((p) => p.id === refLine.aId)?.label}–
                {points.find((p) => p.id === refLine.bId)?.label}
              </span>
              <button className="btn" style={{ background: "#ef4444" }} onClick={clearRefLine}>
                Clear
              </button>
            </>
          )}
          ...
          <button
            ...
            style={{ background: mode === "tie" ? "#0ea5e9" : "#64748b" }}
          >
            Tie line
          </button>
          ...
          <button
            ...
            style={{ background: mode === "circle" ? "#0ea5e9" : "#64748b" }}
          >
            Circle
          </button>
                  ...
                  <tr key={p.id}>
                    ...
                    <td style={td}>
                      <button
                        ...
                        onClick={() => setPoints((arr) => arr.filter((x) => x.id !== p.id))}
                      >
                        Del
                      </button>
                    </td>
          ...
          angles.map((a) => (
            <div key={a.id} className="row" style={{ justifyContent: "space-between" }}>
              <div className="small">
                ...
                {angleDeg(
                  points.find((p) => p.id === a.a) || { x: 0, y: 0 },
                  points.find((p) => p.id === a.b) || { x: 0, y: 0 },
                  points.find((p) => p.id === a.c) || { x: 0, y: 0 }
                )}
                °
              </div>
              <button
                className="btn"
                style={{ background: "#ef4444" }}
                onClick={() => setAngles((ag) => ag.filter((x) => x.id !== a.id))}
              >
                Del
              </button>
            </div>
                    ...
                    <tr key={t.id}>
                      ...
                      <td style={td}>
                        <button
                          className="btn"
                          style={{ background: "#ef4444" }}
                          onClick={() => setTies((ts) => ts.filter((x) => x.id !== t.id))}
                        >
                          Del
                        </button>
                      </td>
          ...
          circles.map((c, idx) => (
            <div key={c.id} className="row" style={{ justifyContent: "space-between" }}>
              ...
              <button
                className="btn"
                style={{ background: "#ef4444" }}
                onClick={() => setCircles((cs) => cs.filter((x) => x.id !== c.id))}
              >
                Del
              </button>
            </div>
