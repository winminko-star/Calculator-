...
function drawPreview(canvas, state) {
  ...
  points.forEach(p=>{
    ...
    ctx.beginPath(); ctx.arc(X(p.x), Y(p.y), 4, 0, Math.PI*2); ctx.stroke(); ctx.fill();
...
function CanvasPreview({ state }) {
  ...
  useEffect(()=>{ drawPreview(ref.current, state); }, [state]);
  return (
    <canvas
      ref={ref}
...
export default function AllReview() {
  ...
  const openIn2D = (it) => {
    const payload = it?.state && it.state.points && it.state.lines
      ? it.state
      : null;

    if (!payload) {
      alert("This record has no drawable data (points/lines). Please save again from 2D.");
      return;
    }
    try {
        ...
        {items.map(it=>(
          <div key={it.id} className="card" style={{ padding:12, marginBottom:10 }}>
            ...
            <div className="small" style={{ marginTop:6 }}>
              ...
              {" "}{it.meta?.triples ?? 0} ∠ · unit {it.unitLabel || "mm"}
            </div>
